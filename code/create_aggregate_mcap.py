...
for filename in os.listdir(prices_folder):
    if filename.endswith('.csv'):
        ...
        # Keep only the last entry per quarter for each ticker
        df = df.sort_values('timestamp').groupby('quarter').tail(1)
...
# Group by quarter and calculate the total market cap
quarterly_market_cap = all_data.groupby('quarter')['marketCap'].sum().reset_index()

# Convert the period back to a datetime for output
quarterly_market_cap['quarter'] = quarterly_market_cap['quarter'].dt.to_timestamp()

# Save the result to a CSV file
output_file = 'aggregate_market_cap.csv'
quarterly_market_cap.to_csv(output_file, index=False)

print(f"Aggregate market cap data has been saved to {output_file}")
